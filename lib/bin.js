...
function notify(event, useBrowser) {
	switch (event.type) {
		...
		case "device_code":
			...
			if (event.expiresInSeconds !== void 0 && Number.isFinite(event.expiresInSeconds) && event.expiresInSeconds > 0) process.stdout.write(`This code expires in ${String(Math.ceil(event.expiresInSeconds / 60))} minutes.\n`);
