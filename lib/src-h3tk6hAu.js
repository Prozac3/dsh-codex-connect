...
var OpenAICodexWebAuth = class {
	...
	async signIn(method = "browser") {
		...
		const challenge = this.challenge === void 0 ? await new Promise((resolve, reject) => {
	...
	pendingDeviceCode() {
		return this.challenge !== void 0 && "verificationUri" in this.challenge ? this.challenge : void 0;
	}
	...
	submitCallback(callbackUrl) {
		...
		if (this.disposed || this.transition !== void 0 || this.cancellation?.signal.aborted !== false || pending === void 0 || this.challenge === void 0 || !("url" in this.challenge)) return 409;
	...
	start(method) {
		...
		const login = () => loginOpenAICodex({
			...
			prompt: (prompt) => prompt.type === "select" ? Promise.resolve(method) : prompt.type === "manual_code" ? this.waitForManualPrompt(prompt, cancellation.signal) : waitForPromptAbort(prompt, cancellation.signal),
			notify: (event) => {
		...
		this.operation = (this.proxyManager?.run(this.resolveProxyUrl(), login) ?? login()).then(async () => {
			...
			if (this.challenge === void 0) {
				const error = /* @__PURE__ */ new Error("OpenAI Codex sign-in finished without a login challenge");
			...
			this.loginMethod = void 0;
		});
	}
	onEvent(event) {
		if (event.type === "auth_url") {
			if (this.loginMethod === "device_code") {
				this.cancelSignIn(/* @__PURE__ */ new Error("OpenAI Codex returned a browser challenge for device-code login"));
				return;
			}
		...
		if (this.loginMethod !== "device_code") {
			this.cancelSignIn(/* @__PURE__ */ new Error("OpenAI Codex returned a device-code challenge for browser login"));
			return;
		}
		let url;
		try {
			...
			this.cancelSignIn(/* @__PURE__ */ new Error("OpenAI returned an invalid device verification URL"));
			return;
		}
		if (url.protocol !== "https:" || url.username !== "" || url.password !== "" || url.hash !== "" || event.userCode.trim() === "" || event.userCode.length > 256) {
			this.cancelSignIn(/* @__PURE__ */ new Error("OpenAI returned an unsafe device-code challenge"));
			return;
		}
		if (event.expiresInSeconds !== void 0 && (!Number.isFinite(event.expiresInSeconds) || event.expiresInSeconds <= 0)) {
			this.cancelSignIn(/* @__PURE__ */ new Error("OpenAI returned an invalid device-code expiry"));
			return;
		}
		const challenge = {
		...
		const routes = [
			...
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					...
					if (!await authorize(req, res)) return;
					try {
						json$4(res, 200, await auth.signIn("device_code"));
					} catch (error) {
						json$4(res, 500, { error: publicAuthError(error) });
					}
				}
